...
function LuxuryPieceItem({
  ...
  const progressPercentage = (Number(currentPieceCount) / Number(targetPieceCount)) * 100 ?? null;

  return (
    <div className={classNames(style.container, status === '모집 중' && style['border-highlight'])}>
      <div className={style['pricing-container']}>
        {syrupOnly && (
          <img src={imageSrc} className={style['syrup-only-badge']} alt="시럽독점" width={45} height={45} />
        )}
        <div className={style['image-container']}>
          ...
          ></div>
          <img
            src="../static/rolex.png"
            className={classNames(status === '모집 중' ? style.image : style['image-blurred'])}
            alt={upperTitle}
          />
          {status === '모집일자 미정' && (
            <Text typography="t900" color="#fff" className={style['image-sub-text']}>
              Coming Soon
            </Text>
          )}
          {status === '모집일자 D-DAY' && (
        ...
        <div className={style['info-container']}>
          ...
          {price && (
            <div className={style['pricing-text-container']}>
              <span>
                모집가격 <span className={style['pricing-text--small']}>(1조각)</span>
              </span>
              <p className={style['pricing-text']}>{price}원</p>
            </div>
          )}
          {expectedReturnRate && (
            <div className={style['pricing-text-container']} id="pricing-text-container">
              <span>
                예상 수익률 <span className={style['pricing-text--small']}>(트레져러)</span>
              </span>
              <p
                id="pricing-text"
                className={classNames(
                  style['pricing-text'],
                  Number(expectedReturnRate) > 0 ? style['pricing-text-plus'] : style['pricing-text-minus'],
                )}
              >
                {Number(expectedReturnRate) > 0 ? `+${expectedReturnRate}` : `${expectedReturnRate}`}%
              </p>
            </div>
          )}
        </div>
        {status === '모집 중' && (
          <div className={style['status-badge-ongoing']} id="status-badge-ongoing">
            모집중
          </div>
        )}
        {status === '모집일자 미정' && (
      ...
      {currentPieceCount && targetPieceCount && (
        <>
          <div
            className={classNames(style['progress-container'], status === '모집 중' && style['gather-in-progress'])}
          ...
          >
            {status === '모집 중' && (
              ...
              >
                <img src="/static/Union.svg" className={style['progress-count-message']} id="progress-count-message" />
                <span className={style['progress-count-value']}>{Number(progressPercentage).toFixed(1)}%</span>
              </div>
            )}
            <div id="progress-bar-total" className={style['progress-bar-total']}></div>
            <div
              id="progress-bar"
          ...
          <span className={style['progress-text']} id="progress-text">
            {thousandSeperator(currentPieceCount)} / {thousandSeperator(targetPieceCount)} (조각)
          </span>
        </>
      )}
    </div>
